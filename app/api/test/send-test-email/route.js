...
export async function GET(request) {
  try {
    ...
    const htmlContent = `<html><body style="font-family:Arial"><div style="background:linear-gradient(135deg,#0d1f3c 0%,#1B3A6B 100%);color:white;padding:30px"><h1>Virtus Mirai</h1></div><div style="padding:30px;background:#f9f9f9"><div style="background:white;padding:20px;border-left:4px solid #E8A020"><div style="font-size:18px;font-weight:bold;color:#0d1f3c">${postData.title}</div><p><b>Resumo:</b> ${postData.summary}</p><p><b>Perspectiva:</b> ${postData.comment}</p><p><b>Pergunta:</b> ${postData.question}</p><a href="${dashboardUrl}" style="display:inline-block;background:#0d1f3c;color:white;padding:12px 30px;text-decoration:none;margin-top:20px">Revisar</a></div></div></body></html>`;
