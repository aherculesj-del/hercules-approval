...
const postStorageContent = `// lib/post-storage.js
import fs from 'fs';
...
const searchCronContent = `// app/api/cron/search-news/route.js
...
const publishCronContent = `// app/api/cron/publish-news/route.js
...
function createAllFiles() {
  ...
  fs.writeFileSync(path.join(libDir, 'post-storage.js'), postStorageContent, 'utf8');
  ...
  fs.writeFileSync(path.join(searchDir, 'route.js'), searchCronContent, 'utf8');
  ...
  fs.writeFileSync(path.join(publishDir, 'route.js'), publishCronContent, 'utf8');
  ...
  console.log('\nSEGUNDA-QUINTA as 6h: Busca + Email');
  console.log('TERCA-SEXTA as 8h: Publicacao do dia anterior');
}
